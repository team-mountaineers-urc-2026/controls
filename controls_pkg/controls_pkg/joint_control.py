...
class Manipulator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("gripper_left_button",2)
        self.declare_parameter("gripper_right_button", 1)
        ...
        self.declare_parameter("wrist_pitch_axes", 7)

        self.declare_parameter("finger_extend_button", 3)   # Y
        self.declare_parameter("finger_retract_button", 0)  # A
        ...
        self.wrist_pitch_axes = self.get_parameter("wrist_pitch_axes").get_parameter_value().integer_value
        self.gripper_left_button = self.get_parameter("gripper_left_button").get_parameter_value().integer_value
        self.gripper_right_button = self.get_parameter("gripper_right_button").get_parameter_value().integer_value
        ...
        self.rail_accel = 3000  # deg/s² (tune this)
        self.current_rail_dps = 0
        ...
        self.send_speed_commands(0,0,0,0,0,0,0.5)

    def send_speed_commands(self, rail, shoulder, elbow, roll, pitch, gripper, finger):
        vel_state = JointState()

        vel_state.name = ["linear_rail", "shoulder", "elbow", "wrist_pitch", "wrist_roll", "gripper", "finger"]
        vel_state.velocity = [float(rail), float(shoulder), float(elbow), float(pitch), float(roll), float(gripper), float(finger)]
    ...
    def calculate_motor_speeds(self, joy_msg: Joy):
        ...
        # self.get_logger().info(f"Shoulder Press: {joy_msg.axes[self.shoulder_axes]}, Rail: {joy_msg.buttons[self.rail_left_button] - joy_msg.buttons[self.rail_right_button]}")
        ...
        pitch_dps = int(joy_msg.axes[self.wrist_pitch_axes] * wrist_speed * self.max_dps * -1)
        rail_dps = floor((joy_msg.buttons[self.rail_left_button] - joy_msg.buttons[self.rail_right_button]) * 1000)
        gripper_dps = floor((joy_msg.buttons[self.gripper_left_button] - joy_msg.buttons[self.gripper_right_button]) * 1000)
        if (
            joy_msg.buttons[self.finger_extend_button]
            and
            joy_msg.buttons[self.finger_retract_button]
        ):
            finger_dps = 0.5
        ...
        # # Rail Rampdown Stuff:
        # target_rail_dps = (joy_msg.buttons[self.rail_left_button] - joy_msg.buttons[self.rail_right_button]) * 1000

        # if target_rail_dps == 0:
        #     target_rail_dps = int(-1.5 * self.current_rail_dps)
        # dt = 0.02  # or compute from time

        # delta = target_rail_dps - self.current_rail_dps
        # max_step = self.rail_accel * dt

        # if abs(delta) > max_step:
        #     delta = max_step if delta > 0 else -max_step

        # self.current_rail_dps += delta
        
        
        # rail_dps = int(self.current_rail_dps)
        


        
        # Finished Rail Stuff
        # self.get_logger().info(f"Shoulder: {shoulder_dps}, Rail: {rail_dps}, Rail_Adjusted{rail_dps}")
        ...
        self.send_speed_commands(rail_dps, shoulder_dps, elbow_dps, roll_dps, pitch_dps, gripper_dps, finger_dps)
...
def main(args=None):
    ...
    multi_threaded_executor.add_node(manipulator)
